...
def main():
    ...
    if method in ['ica', 'autodan']:
        ...
        train_dest_path = f"advbench/blank_transfer_train_1.json"
        test_dest_path = f"advbench/blank_transfer_test_{test_number}.json"
        train_goals = data['goal'].tolist()[:1]
        train_dest = {}
        ...
        test_dest = {}
        test_indices = random.sample(range(1, len(data['goal'])), test_number) #AB: turn back to 25 instead of 1
        all_goals = data['goal'].tolist()
